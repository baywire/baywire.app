...
function parseListingHtml(html: string): ListingItem[] {
  ...
  $("a[href*='/event/']").each((_, el) => {
    ...
    if (!href) return;
    const url = absolutize(href, "https://tampa.funnybone.com");
...
function parseEventId(url: string): string | null {
  try {
    const u = new URL(url);
    const m = u.pathname.match(/^\/event\/([^/]+)\/tampa-funny-bone\/?$/);
    return m?.[1] ?? null;
  } catch {
    return null;
  }
